    ...
    for t in range(len(trades)):
        for c in range(len(candles)-LOOKBACK-1,-1,-1):
            if candles['time'][c] >= trades[t]['time']:
                ...
                profit = trades[t]['profit'] / candles['close'][c]
                output = profit
...
def calculate_input(candles):
    ...
    for i in range(NUM_FEATURES):
        summed += candles['open'][i]
        input_vec.append( 100.0 * np.log( summed / candles['close'][0] / (i+1) ) )
    print(input_vec)
    return(input_vec)
...
for t in range(len(data['test_inputs'])):
    ...
    for m in range(NUM_MODELS):
        ...
        if prediction[0][0] > 0.005:
            if data['test_outputs'][t] > 0:
                prediction_balance += 1
            else:
                prediction_balance -= 1
            prediction_tries += 1
        elif prediction[0][0] < -0.005:
            if data['test_outputs'][t] > 0:
                prediction_balance -= 1
                filtered_wrong += 1
            else:
